...
- 현재판보다 앞선 `PARALLEL` 사건 중 이어질 가치가 있는 항목을 최대 5개 남깁니다. 같은 이름·집단의 사건은 최신 내용으로 갱신하고 WORLD·게시판·사건 후보는 타임라인에 넣지 않습니다.
...
3. 목록에서 **월드 비하인드 트래커 0.3.0**을 찾습니다.
4. 설명, 버전, 권한을 확인합니다.
5. **Install**을 누릅니다.
...
0.3.0부터 사용자는 월드 비하인드 트래커의 에이전트 프롬프트 맨 아래에 원하는 섹션을 직접 추가할 수 있습니다. 서버 재시작이나 새 확장 제작은 필요하지 않습니다.

기본 `PARALLEL`과 `WORLD` 섹션은 삭제하지 마세요. 두 기본 섹션은 항상 필요하며, 사용자 섹션은 그 뒤에 추가됩니다.

사용할 수 있는 형식:

```text
[TAG|접이식 카드에 표시할 제목|장소 또는 맥락]
내용 형식과 생성 규칙
```
...
```text
[RUMOR|🗣 주변 소문|현재 지역]
- 신뢰도 또는 출처: 현재 지역에서 들을 수 있는 확인·미확인 소문 한 가지

규칙:
- 확정된 사실과 소문을 구분한다.
- 정보가 없으면 억지로 핵심 사건을 만들지 않는다.

[THREADS|🧩 미해결 복선|현재 RP]
- 단서: 기존 채팅에 실제로 등장했지만 아직 해결되지 않은 의문이나 단서

규칙:
- 새로운 복선을 발명하지 않는다.
- USER가 아직 알 수 없는 비공개 정답은 누설하지 않는다.

[BOARD|📌 마을 게시판|현재 장소]
- 종류: 주민이 실제로 볼 수 있는 공지, 경고, 구인, 분실물 또는 작은 의뢰

규칙:
- 메인 줄거리를 강제하지 않는 선택 가능한 정보만 만든다.
```

UI에는 `🗣 주변 소문`, `🧩 미해결 복선`, `📌 마을 게시판`이 각각 별도의 접이식 카드로 나타납니다.

사용자 정의 섹션 규칙:

- `TAG`는 영문 대문자로 시작해야 합니다.
- 태그에는 영문 대문자, 숫자, `_`, `-`만 사용할 수 있습니다.
- 태그는 서로 중복되면 안 됩니다.
- `PARALLEL`과 `WORLD`를 포함해 한 번의 출력에 최대 12개 섹션을 사용할 수 있습니다.
- 닫는 태그는 필요하지 않습니다.
- 기본 `PARALLEL`과 `WORLD`는 각각 정확히 한 번 유지해야 합니다.
- 프롬프트를 수정하기 전에 기존 프롬프트를 별도 메모장에 복사해 두는 것을 권장합니다.

### 0.2.x에서 업데이트한 사용자의 프롬프트

이전에 에이전트 프롬프트를 직접 수정했다면 패키지 업데이트가 그 내용을 임의로 덮어쓰지 않을 수 있습니다. 새 0.3.0 기본 지침을 사용하려면 다음 순서로 진행하세요.

1. 현재 프롬프트에서 보존할 개인 규칙을 메모장에 복사합니다.
2. 에이전트 편집기의 기본 프롬프트 입력란을 비우고 저장하여 패키지 기본값을 사용합니다.
3. 에이전트 편집기를 다시 열어 0.3.0 기본 지침이 적용됐는지 확인합니다.
4. 프롬프트 맨 아래에 보존해 둔 개인 규칙과 사용자 정의 섹션을 다시 추가합니다.

기본값 복원이나 빈 프롬프트 사용 방식이 화면에서 명확하지 않다면 기존 프롬프트를 지우기 전에 중단하고 도움을 요청하세요.

## WORLD 분류 다양성

기본 프롬프트는 WORLD를 한 실행당 정확히 하나 생성합니다. 0.3.0부터는 🌿 환경과 🔧 작동 방식만 반복하지 않고 다음 분류 전체를 장면에 맞춰 검토합니다.
...
직전 WORLD와 같은 분류는 꼭 필요할 때만 반복하며, 다양성을 위해 정사와 무관한 거대한 설정을 억지로 만들지는 않습니다.

## 변경 내역

### 0.3.0

- 사용자가 프롬프트에서 자유롭게 정의하는 추가 섹션을 지원합니다.
- 사용자 섹션을 자동으로 저장하고 섹션별 접이식 카드로 표시합니다.
- 기본 PARALLEL과 WORLD 형식 및 기존 저장 데이터와의 호환성을 유지합니다.
- WORLD 분류가 환경과 작동 방식에 편중되지 않도록 기본 지침을 개선했습니다.
- USER, 현재 메인 캐릭터, 방금 장면의 등장인물을 장면 밖 진행으로 반복하는 문제를 줄이는 제외 규칙을 추가했습니다.
